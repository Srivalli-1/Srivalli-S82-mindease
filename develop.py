...
def chat():
    ...
    print("✅ Structured Output AI is running inside the virtual environment!")
    print("Start chatting with Gemini! Type 'exit' to quit.")

    system_instructions = (
        "You are a helpful assistant that responds in a structured JSON format. "
        "Always provide your response in the following format: "
        "{'response': '<your reply>', 'sentiment': '<detected sentiment>', 'confidence': '<confidence score>'}"
    )
    ...
    try:
        while True:
            ...
            try:
                ...
                structured_reply = data["candidates"][0]["content"][0]["text"]
                print("Structured Reply:", structured_reply)
